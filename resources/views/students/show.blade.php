...
@section('content')
    ...
    <div
        style="background:var(--card);border-radius:var(--radius);padding:24px;border:1px solid rgba(255,255,255,0.1);margin-bottom:30px">
        <h3 style="margin:0 0 20px 0;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.1)">Student Information
        </h3>

        <h4 style="margin:20px 0 12px 0;font-size:14px;color:var(--accent);text-transform:uppercase;letter-spacing:1px">
            Basic Details</h4>
        ...
        <div class="detail-list">
            <div class="detail-row">
                ...
                <div class="detail-value">
                    {{ $student->dob ? \Carbon\Carbon::parse($student->dob)->format('d M, Y') : 'N/A' }}</div>
            </div>
            <div class="detail-row">
    ...
    <div
        ...
        <table id="paymentTable">
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-group">
                                <a href="{{ route('payments.receipt', $payment) }}" target="_blank" class="action-btn action-btn-view">
                                ...
                                <button type="button" class="action-btn action-btn-soft"
                                    onclick='openEditPaymentModal(@json($payment))'>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                ...
                                <form action="{{ route('payments.destroy', $payment) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="action-btn action-btn-danger" onclick="confirmDelete(this)">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                @empty
                    <tr>
                        <td colspan="8" style="color:var(--muted);padding:20px">No payments recorded</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
@section('extra-styles')
    ...
    <style>
        ...
        #paymentTable tbody tr:nth-child(odd) {
        ...
        function openEditPaymentModal(payment) {
            ...
            details.forEach(d => {
                ...
                if (d.month && d.name.includes(d.month)) {
                    ...
                    if (parts.length > 1) {
                        const suffix = parts[parts.length - 1];
                        if (['1st Quater', '2nd Quater', '3rd Quater', '4th Quater', '1st Half', '2nd Half'].some(
                                s => suffix.includes(s))) {
                            parts.pop();
                            baseName = parts.join(' - ');
                        }
            ...
            if (category === 'Monthly') {
                ...
                details.forEach(d => {
                    ...
                    [' - 1st Quater', ' - 2nd Quater', ' - 3rd Quater', ' - 4th Quater', ' - 1st Half',
                        ' - 2nd Half'
                    ].forEach(suffix => {
