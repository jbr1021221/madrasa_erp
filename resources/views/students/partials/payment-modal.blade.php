...
function updateFeeViews() {
  ...
  combinedOtherFees.forEach((fee, index) => {
      ...
      div.onmouseover = function() { 
        ...
        if(!input.checked) {
          innerDiv.style.background = isPartialFee ? 'rgba(255,193,7,0.15)' : 'rgba(255,255,255,0.05)';
        }
      };
      div.onmouseout = function() { 
        ...
        const innerDiv = this.querySelector('div');
        if(!input.checked) {
          innerDiv.style.background = isPartialFee ? 'rgba(255,193,7,0.08)' : 'rgba(255,255,255,0.02)';
        }
      };
...
function updatePeriod() {
    ...
    if (category === 'Monthly') {
        ...
        orderedMonths.forEach((monthObj, index) => {
            ...
            let badge = '';
            if (status === 'paid') badge = '<span style="font-size:10px; background:rgba(76,175,80,0.2); color:#4caf50; padding:1px 4px; border-radius:3px; margin-left:6px; border:1px solid #4caf50;">PAID</span>';
            else if (status === 'partial') badge = '<span style="font-size:10px; background:rgba(255,193,7,0.2); color:#ffc107; padding:1px 4px; border-radius:3px; margin-left:6px; border:1px solid #ffc107;">PARTIAL</span>';
            ...
            if(actionCell) {
                if(feeType !== 'monthly' || selectedMonthTexts.length > 0) {
                     if(feeType === 'monthly' && validMonthCount === 0) {
                         ...
                         let badgeHtml = '';
                         if (isPartial) badgeHtml += `<span style="font-size:10px; background:rgba(255,193,7,0.2); color:#ffc107; padding:1px 4px; border-radius:3px; margin-right:4px; border:1px solid rgba(255,193,7,0.3);">Partial</span>`;
                         if (feeType === 'monthly' && validMonthCount > 1) badgeHtml += `<span style="font-size:11px; color:var(--muted); margin-right:4px;">(x${validMonthCount})</span>`;
...
function preparePaymentDetails() {
    ...
    if (isMonthly) {
        ...
        if (rowType !== 'monthly') {
            paymentDetails.fee_details.push({
                name: feeName, type: row.dataset.type || 'Other', amount: discountedAmount, original_amount: actual, discount: discount
            });
        } else {
            document.querySelectorAll('.month-checkbox:checked').forEach(cb => {
                ...
                if (!isPaid) {
                    paymentDetails.fee_details.push({
                        name: feeName + ' - ' + monthText, type: 'Monthly', amount: discountedAmount, original_amount: actual, discount: discount, month: cb.value, year: cb.dataset.year
                    });
                }
            });
        }
        ...
        paymentDetails.fee_details.push({
            name: feeName, type: category, amount: discountedAmount, original_amount: actual, discount: discount
        });
    }
  });
  ...
  document.querySelectorAll('.added-other-fee').forEach(row => {
    ...
    let actual = 0, discount = 0, valid = false;
    if (row.dataset.actual) {
    ...
    if (valid) {
        ...
        paymentDetails.fee_details.push({
            name: feeName, type: type, amount: discountedAmount, original_amount: actual, discount: discount
        });
    }
  });
