...
@section('content')
    ...
    @endforeach
        ...
        <div style="position:relative;display:inline-block;">
            <button type="button" onclick="toggleFeeDropdown()" id="feeDropdownBtn"
                style="background:#1b1f22;color:var(--text);border:1px solid rgba(255,255,255,0.15);padding:6px 10px;border-radius:var(--radius);font-size:14px;width:180px;text-align:left;cursor:pointer;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                @php
            ...
            <div id="feeDropdownContent"
                style="display:none;position:absolute;top:100%;left:0;background:#1b1f22;border:1px solid rgba(255,255,255,0.15);border-radius:var(--radius);z-index:1000;padding:10px;width:220px;max-height:300px;overflow-y:auto;box-shadow:0 4px 6px rgba(0,0,0,0.3);">
                @foreach ($feeTypes as $ft)
                    <label
                        style="display:flex;align-items:center;margin-bottom:8px;cursor:pointer;font-size:13px;color:var(--text);">
                ...
                <div
                    style="margin-top:10px;padding-top:10px;border-top:1px solid rgba(255,255,255,0.1);display:flex;justify-content:flex-end;gap:8px;">
                    <button type="button" onclick="clearFeeSelection()"
                        style="background:transparent;border:none;color:var(--muted);font-size:12px;cursor:pointer;">Clear</button>
                    <button type="submit"
                        style="background:var(--primary, #4caf50);color:#fff;border:none;padding:4px 12px;border-radius:4px;font-size:12px;cursor:pointer;">Apply</button>
                </div>
            </div>
        </div>
        ...
        <select name="class_id" id="class_id" onchange="updateSections(); this.form.submit()"
            style="background:#1b1f22;color:var(--text);border:1px solid rgba(255,255,255,0.15);padding:6px 10px;border-radius:var(--radius);font-size:14px;width:180px">
    ...
    <h3 style="margin:16px 0">
        @if (request('status') == 'unpaid')
            Unpaid Students for {{ request('month') ?: date('F') }}, {{ request('year') ?: date('Y') }}
        @else
