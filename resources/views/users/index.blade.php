...
@endif
@if(session('error'))
  <div class="alert error" style="background:rgba(239,68,68,0.2);color:#f87171;padding:12px;border-radius:6px;margin-bottom:16px">
    {{ session('error') }}
  </div>
@endif
...
<table id="datatable">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{ $user->email }}</td>
      <td>
        <span style="padding:4px 8px;border-radius:4px;font-size:12px;text-transform:uppercase;background:{{ $user->role === 'admin' ? 'rgba(227,120,20,0.2)' : 'rgba(255,255,255,0.1)' }};color:{{ $user->role === 'admin' ? 'var(--accent)' : 'var(--text)' }}">
      ...
      <td style="display:flex;gap:6px;justify-content:center">
        ...
        @if($user->id !== auth()->id())
        <form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this user?');" style="display:inline">
            @csrf
            @method('DELETE')
...
#datatable tr:nth-child(odd){background:rgba(255,255,255,0.04)}
#datatable tr:nth-child(even){background:rgba(255,255,255,0.09)}
#datatable tr:hover{background:rgba(227,120,20,0.18);transition:0.2s}
.action-btn{
